...
description:
    - M(community.general.onepassword_info) wraps the C(op    - A fatal error occurs if any of the items being searched for can not be found.
    ...
    - This module was called C(onepassword_facts) before Ansible 2.9, returning C(ansible_facts).
      Note that the M(community.general.onepassword_info) module no longer returns C(ansible_facts      You must now use the C(register) option to use the facts in other tasks.
options:
    search_terms:
        type: list
        description:
